    ...
    results: [
      {
        ...
        message: {
          transformer: (check: CheckovCheck): string => {
            const parts = Object.entries(_.omit(check.check_result, ['result'])).map(([key, value]) => `${_.startCase(key)}: ${typeof value === 'object' ? JSON.stringify(value, null, 2) : String(value)}`);
            parts.push(
